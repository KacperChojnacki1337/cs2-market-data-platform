-- back compat for old kwarg name
    ...
    merge into `steam-tracker-portfolio`.`steam_marts`.`fct_portfolio_history` as DBT_INTERNAL_DEST
        using (
...
group by pd.snapshot_date, r.usd_pln_rate
        ...
        on ((DBT_INTERNAL_SOURCE.snapshot_date = DBT_INTERNAL_DEST.snapshot_date))

    
    when matched then update set
        `snapshot_date` = DBT_INTERNAL_SOURCE.`snapshot_date`,`portfolio_value_usd` = DBT_INTERNAL_SOURCE.`portfolio_value_usd`,`portfolio_value_pln` = DBT_INTERNAL_SOURCE.`portfolio_value_pln`,`total_cost_pln` = DBT_INTERNAL_SOURCE.`total_cost_pln`,`unrealized_pnl_pln` = DBT_INTERNAL_SOURCE.`unrealized_pnl_pln`,`unrealized_pnl_pct` = DBT_INTERNAL_SOURCE.`unrealized_pnl_pct`,`active_positions` = DBT_INTERNAL_SOURCE.`active_positions`,`usd_pln_rate` = DBT_INTERNAL_SOURCE.`usd_pln_rate`
    

    when not matched then insert
        (`snapshot_date`, `portfolio_value_usd`, `portfolio_value_pln`, `total_cost_pln`, `unrealized_pnl_pln`, `unrealized_pnl_pct`, `active_positions`, `usd_pln_rate`)
    values
        (`snapshot_date`, `portfolio_value_usd`, `portfolio_value_pln`, `total_cost_pln`, `unrealized_pnl_pln`, `unrealized_pnl_pct`, `active_positions`, `usd_pln_rate`)
